#!/usr/bin/env python3
"""Apply manual batch 04 enrichment and storyline for ten EWCA Civ 2024 judgments."""
...
BATCH_FILE = ROOT / "storyline_batches" / "ewca_civ_2024_batch_04.json"
...
NOTES = "Manual batch 04 enrichment and storyline, 2026-07-07."
...
CASES: list[dict] = json.loads(
    Path(__file__).with_name("ewca_civ_2024_batch_04_cases.json").read_text(encoding="utf-8")
)
...
def apply_to_judgment(source_path: str, judges: list[str], enrichment: dict) -> None:
    ...
    if json.loads(out_text)["full_text"] != full_text:
        raise ValueError(f"full_text would be truncated in {source_path}")
